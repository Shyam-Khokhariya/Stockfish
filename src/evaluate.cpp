...
namespace Trace {
  ...
  std::ostream& operator<<(std::ostream& os, Score s) {
    os << std::setw(5) << to_cp(mg_value_inverse(s)) << " "
       << std::setw(5) << to_cp(mg_value_inverse(s));
    return os;
  }
...
#undef S
  ...
  Value Evaluation<T>::winnable(Score score) const {
    ...
    if constexpr (T)
    {
        Trace::add(WINNABLE, make_score(u, eg * ScaleFactor(sf) / SCALE_FACTOR_NORMAL - eg_value_inverse(score)));
  ...
  Value Evaluation<T>::value() {
    ...
    auto lazy_skip = [&](Value lazyThreshold) {
        return abs(mg_value_inverse(score) + eg_value_inverse(score)) >   lazyThreshold
